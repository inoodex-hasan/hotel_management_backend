...
@section('title', 'Edit Amenity — '.$hotel->name)

@section('content')
    <div>
        <h2 class="text-xl font-semibold uppercase">Edit Amenity</h2>
        <p class="text-sm text-white-dark">{{ $hotel->name }}</p>
    </div>

    <div class="panel mt-6">
        <form action="{{ route('admin.hotels.amenities.update', [$hotel, $amenity]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="form-group md:col-span-2">
                    ...
                    <div id="amenity-editor-wrapper" class="richtext-wrapper">
                        ...
                        <textarea name="description" id="description" class="hidden">{{ old('description', $amenity->description) }}</textarea>
                    </div>
                </div>
                <div class="form-group flex items-center gap-2">
                    <input type="checkbox" name="is_featured" id="is_featured" value="1" class="form-checkbox" {{ $amenity->is_featured ? 'checked' : '' }} />
