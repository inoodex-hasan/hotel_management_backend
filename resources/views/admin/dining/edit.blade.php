...
@section('title', 'Edit Dining Venue: ' . $dining->name)

@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-xl font-semibold uppercase">Edit Dining Venue</h2>
            <p class="text-xs text-slate-400 mt-1">Updating details for <span class="font-semibold text-primary">{{ $dining->name }}</span></p>
        </div>
    ...
    <div class="panel">
        <form action="{{ route('admin.dining.update', $dining) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-8 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-700/60 pt-5">
                ...
                <button type="submit" class="btn btn-primary">Update Dining Venue</button>
            </div>
        </form>
    </div>
@endsection
